...
module.exports = async function (context) {
  ...
  for (const moduleName of tesseractModules) {
    const modulePath = path.join(appAsarUnpackedDir, 'node_modules', moduleName)
    if (await fs.pathExists(modulePath)) {
      console.log(`  ✅ ${moduleName} unpacked`)
    } else {
      console.error(`  ❌ WARNING: ${moduleName} NOT unpacked!`)

      // Try to copy it manually from source
      const sourceDir = path.join(__dirname, '..', 'node_modules', moduleName)
      if (await fs.pathExists(sourceDir)) {
        console.log(`  📦 Copying ${moduleName} from source...`)
        await fs.copy(sourceDir, modulePath, { overwrite: true })
